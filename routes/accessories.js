...
router.get('/accessories/:id', (req, res) => {
    ...
    const accessoriesSingleId = accessoriesData.find((item) => item._id === accessoriesid)

    if(!accessoriesid){
        res.status(404).json({message:"accessoriesId is not found"})
    }
    res.json(accessoriesSingleId)
})
